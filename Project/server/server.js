...
function hasCompleteLead(lead) {
  return Boolean(lead.fullName && (lead.phone || lead.email) && lead.preferredService && lead.preferredDate);
}

function buildLeadSignature(lead) {
  return [
    cleanText(lead.fullName).toLowerCase(),
    cleanText(lead.phone).toLowerCase(),
    cleanText(lead.email).toLowerCase(),
    cleanText(lead.preferredService).toLowerCase(),
    cleanText(lead.preferredDate).toLowerCase(),
  ].join("|");
}
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    let leadSaved = false;
    if (bookingIntent && hasCompleteLead(session.leadDraft)) {
      const leadSignature = buildLeadSignature(session.leadDraft);
      if (leadSignature && leadSignature === session.lastLeadSignature) {
        return res.json({
          reply,
          topicAllowed,
          suggestedService: {
      ...
      leadSaved = true;
      session.lastLeadSignature = leadSignature;
      session.leadDraft = {
        fullName: "",
        email: "",
        phone: "",
